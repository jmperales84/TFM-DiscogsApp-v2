...
- After describing the configurable options for each microservice, we show how to start them with Docker Compose.  
...
- **Why JARs inside the image?**
  We avoid `configure_spark_with_delta_pip(...)` to keep the container **offline** at runtime and prevent Ivy/Maven issues.  
...
This project is wired with **sequential dependencies** so that long-running ingestion happens first and the rest only starts after it succeeds. In `docker-compose.yml` we use `depends_on` with the condition **`service_completed_successfully`**:
...
- Example query (albums led by Red Garland):
  ...
  RETURN a1, a2, r, p, l
  ```
![Neodash_Query](docs/neodash_query.jpg)
